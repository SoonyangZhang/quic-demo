...
MyQuicServer::MyQuicServer(std::unique_ptr<ProofSource> proof_source,
                       MyQuicBackend *backend)
    : MyQuicServer(std::move(proof_source),
                 backend,
...
MyQuicServer::MyQuicServer(
  ...
  DCHECK(backend_);
    alarm_factory_.reset(new QuicEpollAlarmFactory(&epoll_server_));
    clock_.reset(new QuicEpollClock(&epoll_server_));  
    Initialize();
}

void MyQuicServer::Initialize() {
  ...
  /*std::unique_ptr<CryptoHandshakeMessage> scfg(crypto_config_.AddDefaultConfig(
      QuicRandom::GetInstance(), clock_.get(), crypto_config_options_));*/
}

MyQuicServer::~MyQuicServer() = default;

bool MyQuicServer::CreateUDPSocketAndListen(const QuicSocketAddress& address) {
...
QuicPacketWriter* MyQuicServer::CreateWriter(int fd) {
  return new QuicDefaultPacketWriter(fd);
}

QuicDispatcher* MyQuicServer::CreateQuicDispatcher() {
  return new MyQuicDispatcher(
...
void MyQuicServer::HandleEventsForever() {
  while (true) {
    WaitForEvents();
  }
}
...
void MyQuicServer::PostInnerTask(std::unique_ptr<QueuedTask> task){
    QuicWriterMutexLock lock(&task_mutex_);
